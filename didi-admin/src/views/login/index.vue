...
<script>
  ...
  export default {
    ...
    created() {
      if (this.$store.getters.bs) {
        this.$message({
          message: '请先联系管理员，设置权限！',
          type: 'error'
        })
      }
    },
    methods: {
        ...
        this.$refs.loginForm.validate(valid => {
          if (valid) {
            ...
            this.$store.dispatch('LoginByUsername', this.loginForm).then((res) => {
              Cookies.set('loginInfo', JSON.stringify(res))
              this.getButtonByUserId()
              this.loading = false
              this.$router.push({
                path: '/'
              })
      ...
      getButtonByUserId(){
         const param={systemId:2}
         const arr=[]
         getMenuByUserId(param).then(response => {
             if(response.data){
                   response.data.forEach(item =>{
                      arr.push(item.authoritiesId)
                   })
             }
             Cookies.set('buttonMenu', arr)
         })
      }
    }
  }
