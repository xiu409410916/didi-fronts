<template>
  <div class="app-container calendar-list-container">
    ...
    <el-table :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row
      ...
      <el-table-column min-width="60px" align="center" label="提现订单号">
        <template slot-scope="scope">
          <span>{{scope.row.withdrawNo}}</span>
        </template>
      </el-table-column>
       <el-table-column min-width="60px" align="center" label="提现金额">
        <template slot-scope="scope">
          <span>{{scope.row.amount/100}}</span>
        </template>
      </el-table-column>
       <el-table-column min-width="60px" align="center" label="提现方式">
        <template slot-scope="scope">
          <span>{{scope.row.withdrawType == '0' ? '提到余额' : '提到银行卡' }}</span>
        </template>
      </el-table-column>
      <el-table-column min-width="60px" align="center" label="提现状态">
        <template slot-scope="scope">
